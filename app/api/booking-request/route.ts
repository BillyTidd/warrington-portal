...
export async function POST(request: NextRequest) {
  try {
    ...
    const { customerName, customerEmail, customerPhone, customerCompany, jobEstimate, estimatedCost } = body

    // Validate required fields
    if (!customerName || !customerEmail || !customerPhone || !jobEstimate || !estimatedCost) {
    ...
    const result = await db.collection("booking-requests").insertOne(bookingRequest)
